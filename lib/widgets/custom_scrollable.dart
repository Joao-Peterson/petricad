...
import 'custom_scroll_configuration.dart';

/// Signature used by [CustomScrollable] to build the viewport through which the
/// scrollable content is displayed.
typedef CustomViewportBuilder = Widget Function(BuildContext context, ViewportOffset position);
...
class CustomScrollable extends StatefulWidget {
  ...
  /// the ambient [CustomScrollConfiguration].
  ///
  /// If an explicit [CustomScrollBehavior] is provided to [scrollBehavior], the
  ...
  final CustomViewportBuilder viewportBuilder;
  ...
  /// [CustomScrollIncrementType.page] is 80% of the size of the scroll window, and
  /// for [CustomScrollIncrementType.line], 50 logical pixels.
  final CustomScrollIncrementCalculator? incrementCalculator;
  ...
  ///
  /// [CustomScrollBehavior]s also provide [ScrollPhysics]. If an explicit
  /// [ScrollPhysics] is provided in [physics], it will take precedence,
  /// followed by [scrollBehavior], and then the inherited ancestor
  /// [CustomScrollBehavior].
  final CustomScrollBehavior? scrollBehavior;
  ...
  static CustomScrollableState? of(BuildContext context) {
    final _CustomScrollableScope? widget = context.dependOnInheritedWidgetOfExactType<_CustomScrollableScope>();
  ...
  static bool recommendDeferredLoadingForContext(BuildContext context) {
    final _CustomScrollableScope? widget = context.getElementForInheritedWidgetOfExactType<_CustomScrollableScope>()?.widget as _CustomScrollableScope?;
    if (widget == null) {
      return false;
    }
...
class CustomScrollableState extends State<CustomScrollable> with TickerProviderStateMixin, RestorationMixin
  ...
  final _CustomRestorableScrollOffset _persistedScrollOffset = _CustomRestorableScrollOffset();

  @override
  AxisDirection get axisDirection => widget.axisDirection;

  late CustomScrollBehavior _configuration;
  ...
  void _updatePosition() {
    _configuration = widget.scrollBehavior ?? CustomScrollConfiguration.of(context);
  ...
  Widget build(BuildContext context) {
    ...
    final CustomScrollableDetails details = CustomScrollableDetails(
...
/// like [CustomScrollBehavior] for decorating.
///
...
@immutable
class CustomScrollableDetails {
  /// Creates a set of details describing the [CustomScrollable]. The [direction]
  /// cannot be null.
  const CustomScrollableDetails({
  ...
  /// [CustomScrollable] widget.
  ///
  /// This can be used by [CustomScrollBehavior] to apply a [Scrollbar] to the associated
  /// [CustomScrollable].
  final ScrollController controller;
}

/// With [_CustomScrollSemantics] certain child [SemanticsNode]s can be
...
/// scrollable children.
class _CustomScrollSemantics extends SingleChildRenderObjectWidget {
  const _CustomScrollSemantics({
    Key? key,
    required this.position,
  ...
  @override
  _CustomRenderScrollSemantics createRenderObject(BuildContext context) {
    return _CustomRenderScrollSemantics(
      position: position,
  ...
  @override
  void updateRenderObject(BuildContext context, _CustomRenderScrollSemantics renderObject) {
    renderObject
      ..allowImplicitScrolling = allowImplicitScrolling
      ..position = position
      ..semanticChildCount = semanticChildCount;
...
class _CustomRenderScrollSemantics extends RenderProxyBox {
  _CustomRenderScrollSemantics({
...
/// increment given a [CustomScrollIncrementDetails].
///
/// This function is used as the type for [CustomScrollable.incrementCalculator],
/// which is called from a [ScrollAction].
typedef CustomScrollIncrementCalculator = double Function(CustomScrollIncrementDetails details);

/// Describes the type of scroll increment that will be performed by a
/// [ScrollAction] on a [CustomScrollable].
///
/// This is used to configure a [CustomScrollIncrementDetails] object to pass to a
/// [CustomScrollIncrementCalculator] function on a [CustomScrollable].
///
...
/// expectations.
/// {@endtemplate}
enum CustomScrollIncrementType {
  /// Indicates that the [CustomScrollIncrementCalculator] should return the scroll
  ...
  /// [CustomScrollIncrementCalculator] function to decide what that means for a
...
/// of a [CustomScrollIncrementCalculator] function, as well as the current metrics
/// for the scrollable.
class CustomScrollIncrementDetails {
  /// A const constructor for a [CustomScrollIncrementDetails].
  ///
  /// All of the arguments must not be null, and are required.
  const CustomScrollIncrementDetails({
    required this.type,
  ...
  final CustomScrollIncrementType type;
...
/// specified.
class CustomScrollIntent extends Intent {
  /// Creates a const [CustomScrollIntent] that requests scrolling in the given
  /// [direction], with the given [type].
  const CustomScrollIntent({
    required this.direction,
    this.type = CustomScrollIncrementType.line,
  });
...
/// [primaryFocus] by the amount configured in the [CustomScrollIntent] given to it.
///
...
/// for a [CustomScrollIntent.type] set to [CustomScrollIncrementType.page] is 80% of the
/// size of the scroll window, and for [CustomScrollIncrementType.line], 50 logical
/// pixels.
class CustomScrollAction extends Action<CustomScrollIntent> {
  @override
  bool isEnabled(CustomScrollIntent intent) {
  ...
  double _calculateScrollIncrement(CustomScrollableState state, { CustomScrollIncrementType type = CustomScrollIncrementType.line }) {
    ...
    switch (type) {
      case CustomScrollIncrementType.line:
        return 50.0;
      case CustomScrollIncrementType.page:
  ...
  double _getIncrement(CustomScrollableState state, CustomScrollIntent intent) {
  ...
  @override
  void invoke(CustomScrollIntent intent) {
...
// [enabled].
class _CustomRestorableScrollOffset extends RestorableValue<double?> {
  @override
